...
(function () {
	$.fn.rateDefaults = {
		...
		textList: ["", "", "Medium", "Good", "Excellent"], // 文字描述
		theme: "#FFB800", // 主题色
		...
		countValue: function () {
			...
			if (_this.value == parseInt(_this.value)) {
				...
				returnValue = {
					...
					float:
						(parseFloat(_this.value) * 1000 - parseInt(_this.value) * 1000) /
						1000,
				};
			}
			return returnValue;
		...
		countText: function () {
			...
			if (_this.decimal || _this.half) {
				_this.value > parseInt(_this.value) &&
					_this.value >
						(curRound
							? (curRound * _this.length) / _this.textList.length
							: _this.length / _this.textList.length) &&
					curIndex++; // 解决value为小数的时候
			}
		...
		render: function (renderBox, type) {
			...
			var _this = this,
				...
				rateHtmlTpl =
					...
					_this.customClass +
					(_this.hover ? '" title="' + _this.value + '"' : '"') +
					'><div class="rate_box clearfix">{{rateHtmlTpl}}</div>{{textTpl}}</div>',
				rateItemIcon = "";
			var rateItemStyle =
				(_this.theme ? "color:" + _this.theme : "") +
				";" +
				(_this.gutter ? "margin: 0 " + _this.gutter : "") +
				";";
			var rateItemIconStyle =
				(_this.size ? "font-size:" + _this.size : "") + ";";

			if (type * 1 === 0) {
				rateItemIcon =
					'<svg class="icon" style="' +
					rateItemIconStyle +
					'" aria-hidden="true"><use xlink:href="#{{iconname}}"></use></svg>';
			} else if (type * 1 === 1) {
				rateItemIcon =
					'<i class="iconfont {{iconname}}" style="' +
					rateItemIconStyle +
					'"></i>';
			} else if (type * 1 === 2) {
				rateItemIcon =
					'<i class="iconfont icon-rate-item" style="' +
					rateItemIconStyle +
					'">{{iconname}}</i>';
			}

			var rateItemTpl =
				'<div class="rate_item {{selectClass}}" style="' +
				rateItemStyle +
				'">' +
				rateItemIcon +
				"</div>";
				...
				if (_this.decimal || _this.half) {
					...
					if (i < valueObj.int) {
						rateHtml =
							rateHtml +
							rateItemTpl
								.replace("{{selectClass}}", _this.selectClass)
								.replace("{{iconname}}", _this.getIconName(10, type));
						...
						rateHtml =
							rateHtml +
							rateItemTpl
								.replace("{{selectClass}}", _this.incompleteClass)
								.replace("{{iconname}}", _this.getIconName(index, type));
					} else {
						rateHtml =
							rateHtml +
							rateItemTpl
								.replace("{{selectClass}}", "")
								.replace("{{iconname}}", _this.getIconName(0, type));
					}
				} else {
					if (i < valueObj.int) {
						rateHtml =
